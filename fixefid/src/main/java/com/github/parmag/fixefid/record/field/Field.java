...
public class Field {
	...
	public Field(String name, int index, int subIndex, int occurIndex, FieldType type, int len, FieldMandatory mandatory, RecordWay recordWay, 
		...
		if (FieldType.N.equals(type) && len > 19) {
			throw new FieldException(name, ErrorCode.FE12, "The field name=[" + name + "] type=[" + type.name() + "] has wrong length=[" + len + "]. Len <= 19 expected.");
		}
		...
		if (!FieldType.N.equals(type) && decimalFormat != null) {
			throw new FieldException(name, ErrorCode.FE13, "The field name=[" + name + "] type=[" + type.name() + " has wrong extended property decimal format. Type " + FieldType.N.name() + " expected.");
		}
		
		if (!FieldType.AN.equals(type) && dateFormat != null) {
			throw new FieldException(name, ErrorCode.FE14, "The field name=[" + name + "] type=[" + type.name() + " has wrong extended property date format. Type " + FieldType.AN.name() + " expected.");
		}
		
		if (!FieldType.AN.equals(type) && booleanFormat != null) {
			throw new FieldException(name, ErrorCode.FE15, "The field name=[" + name + "] type=[" + type.name() + " has wrong extended property boolean format. Type " + FieldType.AN.name() + " expected.");
		}
		
		if (!FieldType.AN.equals(type) && customFormat != null) {
			throw new FieldException(name, ErrorCode.FE16, "The field name=[" + name + "] type=[" + type.name() + " has wrong extended property custom format. Type " + FieldType.AN.name() + " expected.");
		}
		...
		if ((dateFormat != null && booleanFormat != null) || (customFormat != null && booleanFormat != null)
			...
			throw new FieldException(name, ErrorCode.FE17, "The field name=[" + name + "] type=[" + type.name() + " has wrong too much extended properties boolean format, date format and custom format. Only one is expected.");
		}
		
		if ((!FieldType.N.equals(type) || decimalFormat == null) && removeDecimalSeparator) {
			throw new FieldException(name, ErrorCode.FE18, "The field name=[" + name + "] type=[" + type.name() + " has wrong extended property=[" + 
					FieldExtendedPropertyType.REMOVE_DECIMAL_SEPARATOR + "]. Type " + FieldType.N.name() + " expected.");
		}
		
		if (padStr.length() != 1) {
			throw new FieldException(name, ErrorCode.FE19, "The field name=[" + name + "] type=[" + type.name() + " has extended property=[" + 
	...
	public String getValueAsString() throws FieldException {
		...
		if (isString()) {
			...
			throw new FieldException(name, ErrorCode.FE20, parseTypeErrorMessage("String"));
		}
	}
		...
		if (isNumericSpace(value)) {
			...
			if (isLong() || isLenNormalized()) {
				try {
					...
					throw new FieldException(name, ErrorCode.FE21, parseTypeErrorMessage("Long"));
				}
			} else {
				throw new FieldException(name, ErrorCode.FE22, parseTypeErrorMessage("Long"));
			}
		}
	}
		...
		if (isNumericSpace(value)) {
			...
			if (isInteger()) {
				try {
					...
					throw new FieldException(name, ErrorCode.FE23, parseTypeErrorMessage("Integer"));
				}
			} else {
				throw new FieldException(name, ErrorCode.FE24, parseTypeErrorMessage("Integer"));
			}
		}
	}
		...
		if (isNumericSpace(value)) {
			...
			if (isDouble() || isLenNormalized()) {
				...
				try { 
					...
					throw new FieldException(name, ErrorCode.FE25, parseTypeErrorMessage("Double"));
				}
			} else {
				throw new FieldException(name, ErrorCode.FE26, parseTypeErrorMessage("Double"));
			}
		}
	}
		...
		if (isNumericSpace(value)) {
			...
			if (isFloat()) {
				...
				try { 
					...
					throw new FieldException(name, ErrorCode.FE27, parseTypeErrorMessage("Float"));
				}
			} else {
				throw new FieldException(name, ErrorCode.FE28, parseTypeErrorMessage("Float"));
			}
		}
	}
		...
		if (isNumericSpace(value)) {
			...
			if (isBigDecimal()) {
				...
				try { 
					...
					throw new FieldException(name, ErrorCode.FE29, parseTypeErrorMessage("BigDecimal"));
				}
				...
				throw new FieldException(name, ErrorCode.FE30, parseTypeErrorMessage("BigDecimal"));
			}
		}
	}
	...
	public Date getValueAsDate() throws FieldException {
		...
		if (isDate()) {
			try {
				...
				throw new FieldException(name, ErrorCode.FE31, parseTypeErrorMessage("Date"));
			}
		} else {
			throw new FieldException(name, ErrorCode.FE32, parseTypeErrorMessage("Date"));
		}
	}
	...
	public Boolean getValueAsBoolean() throws FieldException {
		...
		if (isBoolean()) {
			...
			throw new FieldException(name, ErrorCode.FE33, parseTypeErrorMessage("Boolean"));
		}
	}
	...
	public void setValue(Long value) throws FieldException {
		...
		if (isLong() || isLenNormalized()) {
			...
			throw new FieldException(name, ErrorCode.FE34, parseTypeErrorMessage("Long"));
		}
	}
	...
	public void setValue(Integer value) throws FieldException {
		...
		if (isInteger()) {
			...
			throw new FieldException(name, ErrorCode.FE35, parseTypeErrorMessage("Integer"));
		}
	}
	...
	public void setValue(Double value) throws FieldException {
		...
		if (isDouble() || isLenNormalized()) {
			...
			throw new FieldException(name, ErrorCode.FE36, parseTypeErrorMessage("Double"));
		} 
	}
	...
	public void setValue(Float value) throws FieldException {
		...
		if (isFloat()) {
			...
			throw new FieldException(name, ErrorCode.FE37, parseTypeErrorMessage("Float"));
		} 
	}
	...
	public void setValue(BigDecimal value) throws FieldException {
		...
		if (isBigDecimal()) {
			...
			throw new FieldException(name, ErrorCode.FE38, parseTypeErrorMessage("BigDecimal"));
		} 
	}
	...
	public void setValue(Date value) throws FieldException {
		...
		if (isDate()) {
			...
			throw new FieldException(name, ErrorCode.FE39, parseTypeErrorMessage("Date"));
		}
	}
	...
	public void setValue(Boolean value) throws FieldException {
		...
		if (isBoolean()) {
			...
			throw new FieldException(name, ErrorCode.FE40, parseTypeErrorMessage("Boolean"));
		}
	}
	...
	protected void checkValidationInfo() {
		...
		if (FieldValidationInfo.RecordFieldValidationStatus.ERROR.equals(validationInfo.getValidationStatus())) {
			throw new FieldException(name, ErrorCode.FE10, "Validation Code " + validationInfo.getValidationCode() +
