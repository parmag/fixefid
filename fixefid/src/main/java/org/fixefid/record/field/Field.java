...
import org.fixefid.record.format.BooleanFormat;
...
public class Field {
	 ...
	 */
	public Field(String name, int index, FieldType type, int len, FieldMandatory mandatory, RecordWay recordWay, 
	...
	protected void validValue() {
		...
		if (FieldType.N.equals(type)) {
			if (validator != null) {
				validationInfo = validator.valid(name, index, type, mandatory, value, fieldExtendedProperties);
			} else {
				if (isDouble()) {
					try {
						getValueAsDouble();
	 ...
	 * character without accent. For example the character à is replaced with the character a
	 */
